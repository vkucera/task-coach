...
class ViewAllTasksTest(ViewMenuTestCase):
    ...
    def createFilteredTaskList(self):
        return task.filter.SearchFilter(task.filter.CategoryFilter(task.filter.ViewFilter(task.TaskList(), settings=self.settings), settings=self.settings), settings=self.settings)
