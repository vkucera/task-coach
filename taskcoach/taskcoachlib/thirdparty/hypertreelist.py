...
Usage example::
    ...
    class MyFrame(wx.Frame):
        ...
        def __init__(self, parent):
            ...
            parent = tree_list.Append(root, "First child", ct_type=1)
            child = tree_list.Append(parent, "First Grandchild", ct_type=1)
            
            tree_list.Append(root, "Second child", ct_type=1)
...
class TreeListColumnInfo(object):
    ...
    def __init__(
        ...
        if isinstance(input, str):
            ...
            if colour is None:
                self._colour = wx.SystemSettings.GetColour(
...
class TreeListHeaderWindow(wx.Window):
    ...
    def OnPaint(self, event):
        ...
        for i in range(numColumns):
            ...
            if self._headerCustomRenderer != None:
                self._headerCustomRenderer.DrawHeaderButton(
                    dc, rect, flags, params
                )
            else:
    ...
    def OnMouse(self, event):
        ...
        if self._isDragging:
            ...
            self.SendListEvent(
                wx.wxEVT_COMMAND_LIST_COL_DRAGGING, event.GetPosition()
            )
    ...
    def AddColumn(
        ...
        colInfo = TreeListColumnInfo(
            text, width, flag, image, shown, colour, edit
        )
...
class TreeListItem(GenericTreeItem):
    ...
    def HitTest(self, point, theCtrl, flags, column, level):
        ...
        if not theCtrl.HasAGWFlag(wx.TR_HIDE_ROOT) or level > 0:
            ...
            if point.y >= self._y and point.y <= self._y + h:
                ...
                for i in range(maincol):
                    ...
                    if (point.x > (self._text_x + self._width)) and (
                        point.x <= end
                    ):
        ...
        for child in self._children:
            hit, flags, column = child.HitTest(
                point, theCtrl, flags, column, level + 1
            )
            if hit:
    ...
    def GetText(self, column=None):
        ...
        column = (
            column is not None and [column] or [self._owner.GetMainColumn()]
        )[0]
    ...
    def SetImage(self, column, image, which):
        ...
        if column == self._owner.GetMainColumn():
            ...
            self._col_images.extend(
                [_NO_IMAGE] * (column - len(self._col_images) + 1)
            )
            self._col_images[column] = image
    ...
    def GetWindowEnabled(self, column=None):
        ...
        if not self._wnd[column]:
            raise Exception(
                "\nERROR: This Item Has No Window Associated At Column %s"
                % column
            )
        ...
        **kwargs
        ...
        if column > 0:
            ...
            if checkimage is not None:
                wcheck, hcheck = self._owner._imageListCheck.GetSize(
                    checkimage
                )
                wcheck += 2 * _MARGIN
...
class TreeListMainWindow(CustomTreeCtrl):
    ...
    def SetItemImage(
        self, item, image, column=None, which=wx.TreeItemIcon_Normal
    ):
        """
        Sets the item image for a particular item state.
    ...
    def DoInsertItem(
        ...
        item = TreeListItem(
            self, parent, arr, ct_type, wnd, image, selImage, data
        )
    ...
    def AddRoot(
        self, text, ct_type=0, wnd=None, image=-1, selImage=-1, data=None
    ):
        """
    ...
    def Delete(self, item):
        ...
        if self._editCtrl != None and self.IsDescendantOf(
        ...
        if item._y < start_y + 3:
            ...
            self.SetScrollbars(
                ...
                (
                    yUnit
                    and [
                        (item._y + self.GetLineHeight(item) - client_h) / yUnit
                    ]
                    or [0]
                )[0],
            )
    ...
    def AdjustMyScrollbars(self):
        ...
        if self._anchor:
            ...
            self.SetScrollbars(
                xUnit, yUnit, x // xUnit, y // yUnit, x_pos, y_pos
            )
        else:
    ...
    def PaintItem(self, item, dc):
            ...
            if alignment != wx.ALIGN_LEFT and len(txt):
                ...
                for t in txt:
                    ...
                    if alignment == wx.ALIGN_CENTER:
                        plus /= 2
                    dc.DrawLabel(
                        t, wx.Rect(textrect.X + plus, yorigin, w, yorigin + h)
                    )
                    yorigin += h
                return
        ...
        img_extraH = (
            total_h > self._imgHeight
            and [(total_h - self._imgHeight) / 2]
            or [0]
        )[0]
        x_colstart = 0
        ...
        for i in range(self.GetColumnCount()):
            ...
            if i == self.GetMainColumn():
                ...
                if item.GetType() != 0 and self._imageListCheck:
                    checkimage = item.GetCurrentCheckedImage()
                    wcheck, hcheck = self._imageListCheck.GetSize(
                        item.GetType()
                    )
                else:
                    wcheck, hcheck = 0, 0
            ...
            text = item.GetText(i)
            alignment = (
                self._owner.GetHeaderWindow().GetColumn(i).GetAlignment()
            )
            ...
            if wcheck:
                ...
                imglist.Draw(
                    ...
                    item.GetY()
                    + ((total_h > hcheck) and [(total_h - hcheck) / 2] or [0])[
                        0
                    ]
                    + 1,
            ...
            text_extraH = (
                total_h > text_h and [(total_h - text_h) / 2] or [0]
            )[0]
    ...
    def PaintLevel(self, item, dc, level, y, x_maincol):
        ...
        if self.IsExposed(
            exposed_x, exposed_y, _MAX_WIDTH, h + draw_row_lines
        ):
            if draw_row_lines:
            ...
            clip_width = (
                self._owner.GetHeaderWindow()
                .GetColumn(self._main_column)
                .GetWidth()
            )
            ...
            if (
                item.HasPlus() and self.HasButtons()
                ...
                if self._imageListButtons:
                    ...
                    if item.IsSelected():
                        image += (
                            wx.TreeItemIcon_Selected - wx.TreeItemIcon_Normal
                        )
                    xx = x - self._btnWidth2 + _MARGIN
                    yy = y_mid - self._btnHeight2
                    dc.SetClippingRegion(
                        xx, yy, self._btnWidth, self._btnHeight
                    )
                    ...
                    dc.SetClippingRegion(
                        x_maincol + _MARGIN, y_top, clip_width, h
                    )
                    ...
                    flag = (
                        item.IsExpanded() and [wx.CONTROL_EXPANDED] or [0]
                    )[0]
        ...
        if item.IsExpanded():
            ...
            for child in item.GetChildren():
                ...
                y, x_maincol = self.PaintLevel(
                    child, dc, level + 1, y, x_maincol
                )
    ...
    def OnPaint(self, event):
        ...
        if self._imageListCheck:
            self._checkWidth, self._checkHeight = self._imageListCheck.GetSize(
                0
            )
    ...
    def EditLabel(self, item, column=None):
        ...
        te = TreeEvent(
            wx.wxEVT_COMMAND_TREE_BEGIN_LABEL_EDIT, self._owner.GetId()
        )
    ...
    def OnAcceptEdit(self, value):
        ...
        le = TreeEvent(
            wx.wxEVT_COMMAND_TREE_END_LABEL_EDIT, self._owner.GetId()
        )
    ...
    def OnMouse(self, event):
        ...
        if (
            ...
            if self._editCtrl != None and (
                item != self._editCtrl.item()
                or column != self._editCtrl.column()
            ):
                self._editCtrl.StopEditing()
        ...
        if (
            ...
            if hoverItem is not None:
                ...
                hevent = TreeEvent(
                    wx.wxEVT_COMMAND_TREE_ITEM_GETTOOLTIP, self.GetId()
                )
                hevent.SetItem(hoverItem)
                hevent.SetEventObject(self)

                if (
                    self.GetEventHandler().ProcessEvent(hevent)
                    and hevent.IsAllowed()
                ):
        ...
        if event.Dragging():
            ...
            if (
                self.GetEventHandler().ProcessEvent(nevent)
                and nevent.IsAllowed()
            ):
            ...
            nevent = TreeEvent(
                wx.wxEVT_COMMAND_TREE_END_DRAG, self._owner.GetId()
            )
            nevent.SetEventObject(self._owner)
        ...
        if (
            item == None or not self.IsItemEnabled(item)
        ...
        if event.RightUp():
            ...
            if self._lastOnSame:
                if (
                    ...
                    and self._owner.GetHeaderWindow().IsColumnEditable(
                        self._curColumn
                    )
                    and flags
                    & (
    ...
    def CalculatePositions(self):
        ...
        self.CalculateLevel(
            self._anchor, dc, 0, y, x_colstart
        )  # start recursion
    ...
    def GetItemWidth(self, item, column):
        ...
        if column == self.GetMainColumn():
            ...
            while parent and (
                not self.HasAGWFlag(wx.TR_HIDE_ROOT) or (parent != root)
            ):
                level += 1
                parent = parent.GetParent()
...
class HyperTreeList(wx.PyControl):
    ...
    def CalculateAndSetHeaderHeight(self):
        ...
        if self._header_win:
            h = max(
                wx.RendererNative.Get().GetHeaderButtonHeight(
                    self._header_win
                ),
    ...
    def SetHeaderFont(self, font):
        ...
        for column in range(self.GetColumnCount()):
            self._header_win.SetColumn(
                column, self.GetColumn(column).SetFont(font)
            )
    ...
    def SetColumnImage(self, column, image):
        ...
        self._header_win.SetColumn(
            column, self.GetColumn(column).SetImage(image)
        )
        self._header_win.Refresh()
    ...
    def SetColumnEditable(self, column, edit):
        ...
        self._header_win.SetColumn(
            column, self.GetColumn(column).SetEditable(edit)
        )
    ...
    def SetColumnAlignment(self, column, flag):
        ...
        self._header_win.SetColumn(
            column, self.GetColumn(column).SetAlignment(flag)
        )
        self._header_win.Refresh()
    ...
    def SetColumnColour(self, column, colour):
        ...
        self._header_win.SetColumn(
            column, self.GetColumn(column).SetColour(colour)
        )
        self._header_win.Refresh()
