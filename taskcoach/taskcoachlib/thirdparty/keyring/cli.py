...
def main(argv=None):
    ...
    if opts.keyring_backend is not None:
        try:
            backend = core.load_keyring(opts.keyring_path, opts.keyring_backend)
    ...
    if kind == 'get':
        password = core.get_password(service, username)
        if password is None:
            return 1
        ...
        password = input_password("Password for '%s' in '%s': " %
                                  (username, service))
        core.set_password(service, username, password)
        return 0
