...
class VCalendarParserTest(test.TestCase):
    ...
    def testEmptyVTodo(self):
        ...
        self.assertEqual(dict(status=0, startDate=date.Date()), 
                         self.parser.tasks[0])
        
    def testSubject(self):
        self.parser.parse(['BEGIN:VTODO', 'SUBJECT:Test', 'END:VTODO'])
        self.assertEqual(dict(status=0, subject='Test', startDate=date.Date()), 
                         self.parser.tasks[0])
        
    def testDueDate(self):
        self.parser.parse(['BEGIN:VTODO', 'DUE:20100101T120000', 'END:VTODO'])
        self.assertEqual(dict(status=0, dueDate=date.Date(2010,1,1), 
                              startDate=date.Date()), 
                         self.parser.tasks[0])
