...
class MainMenu(wx.MenuBar):
    def __init__(self, mainwindow, settings, iocontroller, viewerContainer, 
        ...
        self.Append(EffortMenu(mainwindow, taskFile, viewerContainer), 
                    _('Eff&ort'))
        self.Append(CategoryMenu(mainwindow, taskFile.categories(),
                                 viewerContainer), _('&Category'))
        if settings.getboolean('feature', 'notes'):
            self.Append(NoteMenu(mainwindow, taskFile, viewerContainer), 
                        _('&Note'))
...
class ViewViewerMenu(Menu):
    def __init__(self, mainwindow, settings, viewerContainer, taskFile):
        ...
        viewViewerCommands = [\
            ...
            None,
            ViewViewer(viewer=viewerContainer, menuText=_('&Effort detail'), 
                       helpText=_('Open a new tab with a viewer that displays effort details'), 
                       bitmap='start', viewerClass=viewer.EffortListViewer, 
                       viewerArgs=(tasks, settings), 
                       viewerKwargs=dict(), viewerBitmap='start', 
                       settings=settings),
            ViewViewer(viewer=viewerContainer, menuText=_('&Effort per &day'), 
                       helpText=_('Open a new tab with a viewer that displays effort per day'), 
                       bitmap='date', viewerClass=viewer.EffortPerDayViewer, 
                       viewerArgs=(tasks, settings), 
                       viewerKwargs=dict(), viewerBitmap='date', 
                       settings=settings),
            ViewViewer(viewer=viewerContainer, menuText=_('&Effort per &week'), 
                       helpText=_('Open a new tab with a viewer that displays effort per week'), 
                       bitmap='date', viewerClass=viewer.EffortPerWeekViewer, 
                       viewerArgs=(tasks, settings), 
                       viewerKwargs=dict(), viewerBitmap='date', 
                       settings=settings),
            ViewViewer(viewer=viewerContainer, menuText=_('&Effort per &month'), 
                       helpText=_('Open a new tab with a viewer that displays effort per month'), 
                       bitmap='date', viewerClass=viewer.EffortPerMonthViewer, 
                       viewerArgs=(tasks, settings), 
                       viewerKwargs=dict(), viewerBitmap='date', 
                       settings=settings)]
        if settings.getboolean('feature', 'notes'):
            viewViewerCommands.extend([None, 
...
class TaskMenu(Menu):
    def __init__(self, mainwindow, settings, taskFile, viewerContainer):
        ...
        self.appendUICommands(
            ...
            uicommand.TaskAddAttachment(taskList=tasks, 
                                        viewer=viewerContainer,
                                        settings=settings), 
            uicommand.TaskOpenAllAttachments(viewer=viewerContainer, 
                                             settings=settings))
...
class CategoryMenu(Menu):
    def __init__(self, mainwindow, categories, viewerContainer):
...
class TaskPopupMenu(Menu):
    def __init__(self, mainwindow, settings, tasks, categories, efforts, 
        ...
        commandsToAppend = [\
            ...
            uicommand.TaskAddAttachment(taskList=tasks, viewer=taskViewer,
                                        settings=settings), 
            uicommand.TaskOpenAllAttachments(viewer=taskViewer, 
                                             settings=settings), 
            None, 
...
class NotePopupMenu(Menu):
    def __init__(self, mainwindow, notes, categories, noteViewer):
